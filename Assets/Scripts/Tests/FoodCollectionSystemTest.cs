...
namespace Tests {
	public sealed class FoodCollectionSystemTest {
		...
		public void SetUp() {
			...
			var aiConfig = CreateTestAiConfig();
			_foodGeneratorQueryService = new FoodGeneratorQueryService(_world, itemGeneratorConfig, _itemsConfig, aiConfig);
			_system = new FoodCollectionSystem(_world, _foodGeneratorQueryService, _cellService, _aiService, _itemStorageService);
		}
		...
		[Test]
		public void WhenCollectionCompleted_ShouldMarkFoodItemForConsumption() {
			// Arrange
			var generator = CreateFoodGenerator(new Vector2Int(5, 5));
			...
			var foodItem = CreateFoodItemInStorage(entity);
			...
			// Assert
			Assert.IsTrue(foodItem.Has<AutoConsumeItem>(), "Food item should have AutoConsumeItem");
			Assert.IsFalse(entity.Has<FoodCollectionState>(), "Entity should exit food collection state");
		}

		[Test]
		public void WhenCollectionCompleted_WithNoFoodItem_ShouldExitState() {
			// Arrange
			var entity = CreateBotEntity(new Vector2Int(5, 5));
			entity.Add(new FoodCollectionState { TargetGenerator = Entity.Null });
			entity.Add(new HasAiState());
			entity.Add(new CollectionCompleted { Generator = Entity.Null });
